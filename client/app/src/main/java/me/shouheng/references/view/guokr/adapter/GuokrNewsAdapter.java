package me.shouheng.references.view.guokr.adapter;
...
public class GuokrNewsAdapter extends BaseQuickAdapter<GuokrNews.Result, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, GuokrNews.Result item) {
        ...
        Glide.with(context).load(item.getSmall_image())
                .asBitmap()
                .placeholder(R.drawable.guokr)
                .centerCrop()
                .into((ImageView) helper.getView(R.id.iv_cover));

        Glide.with(context).load(item.getAuthor().getAvatar().getNormal())
                .asBitmap()
                .placeholder(R.drawable.guokr)
                .centerCrop()
